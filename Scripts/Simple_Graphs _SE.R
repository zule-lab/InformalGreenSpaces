...
falaise_simple_summarized <- falaise_subset %>% 
  ...
  summarize(meanT2 = mean(meanT),
            SD = sd(meanT, na.rm = TRUE)) #need both mean and SD for the graph in order to have error bars 
...
falaise_simple_plot <- falaise_simple_summarized %>%
  ...
  geom_errorbar(aes(ymin=meanT2-SD, ymax=meanT2+SD), width=.2, linewidth=1.5, 
                position=position_dodge(0.05))+ #code for the error bars, specified by taking the mean +/- the SD 
  geom_label_repel(data = falaise_simple_summarized, aes(y=meanT2, label= round(meanT2,1), fill = factor(Park), size=5), fontface = 'bold', color = 'white', show.legend = F) + #geom_label_repel function puts a label with the specified information (from a data frame) 
...
falaise_allsensors_simple_summary <- falaise_allsensors_simple_subset %>% 
...
falaise_simple_allsensors_plot <- falaise_allsensors_simple_summary %>%
  ...
  geom_errorbar(aes(ymin=meanT-SD, ymax=meanT+SD), width=.2, linewidth=1.5, 
                position=position_dodge(0.05))+ #code for the error bars, specified by taking the mean +/- the SD 
  geom_label_repel(data = falaise_allsensors_simple_summary, aes(y=meanT, label= round(meanT,1), fill = factor(Sensor_Name), size=5), fontface = 'bold', color = 'white', show.legend = F) + #geom_label_repel function puts a label with the specified information (from a data frame) 
  #Tip: Put repel labels AFTER the code for the points and error bars makes it so the labels appear above the data (unobstructed)!
...
MHM_simple_summarized <- MHM_subset %>% 
  group_by(Park, DayTime) %>% #groups the data in these groups to perform the following operations
  summarize(meanT2 = mean(meanT),
            SD = sd(meanT, na.rm = TRUE))#good if you need to look at mean temp of a day for a specific sensor


MHM_simple_plot <- MHM_simple_summarized %>%
  ggplot() +
  aes(x= Park, y= meanT2, colour = Park) +
  geom_point(size = 4)+ 
  geom_errorbar(aes(ymin=meanT2-SD, ymax=meanT2+SD), width=.2, linewidth=1.5, 
                position=position_dodge(0.05))+ 
  geom_label_repel(data = MHM_simple_summarized, aes(y=meanT2, label= round(meanT2,1), fill = factor(Park), size=5), fontface = 'bold', color = 'white', show.legend = F) +  
