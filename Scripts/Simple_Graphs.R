...
falaise_simple_summarized <- falaise_subset %>% 
...
falaise_simple_plot <- falaise_simple_summarized %>%
  ggplot() +
  ...
  geom_label_repel(data = falaise_simple_summarized, aes(y=meanT, label= round(meanT,1), fill = factor(Park), size=5), 
...
#ggsave("Falaise_Simple_Plot.png", plot=falaise_simple_plot, path = "Graphics", dpi = 600, width = 15, height = 20, units = "cm")
...
falaise_allsensors_simple_summary <- falaise_subset %>% 
  group_by(Sensor_Name, DayTime) %>% #groups the data in these groups to perform the following operations
  summarize(meanT = mean(Temperature),
            SD = sd(Temperature, na.rm = TRUE)) #need both mean and  for the graph in order to have error bars 

falaise_simple_allsensors_plot <- falaise_allsensors_simple_summary %>%
  ggplot() +
  ...
  geom_label_repel(data = falaise_allsensors_simple_summary, aes(y=meanT, label= round(meanT,1), fill = factor(Sensor_Name), size=5), 
...
#ggsave("Falaise_Simple_AllSensors_Plot.png", plot=falaise_simple_allsensors_plot, path = "Graphics", dpi = 600, width = 20, height = 25, units = "cm")
...
MHM_simple_summary <- MHM_subset %>% 
  group_by(Park, DayTime) %>% 
  summarize(meanT = mean(Temperature),
            SD = sd(Temperature, na.rm = TRUE)) 

MHM_simple_plot <- MHM_simple_summary %>%
  ggplot() +
  aes(x=Park , y= meanT, colour = Park) +
  geom_point(size = 4)+ 
  geom_errorbar(aes(ymin=meanT-SD, ymax=meanT+SD), width=.5, linewidth=1.5, 
                position=position_dodge(0.05))+ 
  geom_label_repel(data = MHM_simple_summary, aes(y=meanT, label= round(meanT,1), fill = factor(Park), size=5), 
...
#ggsave("MHM_Simple_Plot.png", plot=MHM_simple_plot, path = "Graphics", dpi = 600, width = 15, height = 20, units = "cm")
...
MHM_ParkSubTypes_Simple_plot <- parksubtypes_combined %>%
  ggplot() +
...
MHM_ParkSubTypes_Simple_plot

#ggsave("MHM_ParkSubTypes_Simple_plot.png", plot=MHM_ParkSubTypes_Simple_plot, path = "Graphics", dpi = 600, width = 20, height = 25, units = "cm")
...
MHM_allsensors_simple_summary <- MHM_subset %>% 
...
MHM_simple_allsensors_plot <- MHM_allsensors_simple_summary %>%
  ...
  geom_point(size = 4)+ 
  geom_errorbar(aes(ymin=meanT-SD, ymax=meanT+SD), width=.5, linewidth=1.5)+ 
  geom_label_repel(data = MHM_allsensors_simple_summary, aes(y=meanT, label= round(meanT,1), fill = factor(Sensor_Name), size=5), 
                   fontface = 'bold', color = 'white', show.legend = F, force=50, segment.color="black", segment.size=1, min.segment.length = 0) + 
...
#ggsave("MHM_Simple_AllSensors_Plot.png", plot=MHM_simple_allsensors_plot, path = "Graphics", dpi = 600, width = 20, height = 25, units = "cm")
...
CdesP_simple_plot <- CdesP_simple_summarized %>%
  ...
  geom_label_repel(data = CdesP_simple_summarized, aes(y=meanT, label= round(meanT,1), fill = factor(Park), size=5), 
...
#ggsave("CdesP_Simple_Plot.png", plot=CdesP_simple_plot, path = "Graphics", dpi = 600, width = 15, height = 20, units = "cm")
...
CdesP_allsensors_simple_summary <- CdesP_subset %>% 
  group_by(Sensor_Name, DayTime) %>% 
  summarize(meanT = mean(Temperature),
            SD = sd(Temperature, na.rm = TRUE)) 
...
CdesP_simple_allsensors_plot <- CdesP_allsensors_simple_summary %>%
  ...
  geom_point(size = 4)+ 
  geom_errorbar(aes(ymin=meanT-SD, ymax=meanT+SD), width=.5, linewidth=1.5)+ 
  geom_label_repel(data = CdesP_allsensors_simple_summary, aes(y=meanT, label= round(meanT,1), fill = factor(Sensor_Name), size=5), 
...
#ggsave("CdesP_Simple_AllSensors_Plot.png", plot=CdesP_simple_allsensors_plot, path = "Graphics", dpi = 600, width = 20, height = 25, units = "cm")
...
TP_simple_plot <- TP_simple_summarized %>%
  ...
  geom_point(size = 4)+ 
  geom_errorbar(aes(ymin=meanT-SD, ymax=meanT+SD), width=0.25, linewidth=1.25)+ 
  geom_label_repel(data = TP_simple_summarized, aes(y=meanT, label= round(meanT,1), fill = factor(Park), size=5), 
                   fontface = 'bold', color = 'white', force=100, segment.color="black", segment.size=1, show.legend = F, min.segment.length = 0) + 
  scale_color_manual(values = c("forestgreen", "gray48")) + 
  scale_fill_manual(values = c("forestgreen", "gray48")) + 
  theme_classic(base_size = 15)+  
...
#ggsave("TP_Simple_Plot.png", plot=TP_simple_plot, path = "Graphics", dpi = 600, width = 20, height = 25, units = "cm")
...
TP_allsensors_simple_summary <- TP_subset %>% 
  group_by(Sensor_Name, DayTime) %>% 
  summarize(meanT = mean(Temperature),
            SD = sd(Temperature, na.rm = TRUE)) 
...
TP_simple_allsensors_plot <- TP_allsensors_simple_summary %>%
  ...
  geom_point(size = 4)+ 
  geom_errorbar(aes(ymin=meanT-SD, ymax=meanT+SD), width=.5, linewidth=1.5)+ 
  geom_label_repel(data = TP_allsensors_simple_summary, aes(y=meanT, label= round(meanT,1), fill = factor(Sensor_Name), size=5), 
                   fontface = 'bold', color = 'white', show.legend = F, force=100, segment.color="black", segment.size=1, min.segment.length = 0) + 
...
#ggsave("TP_Simple_AllSensors_Plot.png", plot=TP_simple_allsensors_plot, path = "Graphics", dpi = 600, width = 20, height = 25, units = "cm")
